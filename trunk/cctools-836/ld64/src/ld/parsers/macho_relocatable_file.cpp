...
//default linux qsort_r can not works here.
//take it from Libc of apple and rename it to qsort_r2 to avoid name conflicts
extern "C"
{
extern void     qsort_r2(void *, size_t, size_t, void *,
...
void Parser<A>::makeSortedSectionsArray(uint32_t array[])
{
	...
	for (uint32_t i=0; i < _machOSectionsCount; ++i)
		array[i] = i;
	::qsort_r2(array, _machOSectionsCount, sizeof(uint32_t), this, &sectionIndexSorter);
...
void Parser<A>::makeSortedSymbolsArray(uint32_t array[], const uint32_t sectionArray[])
{
	...
	::qsort_r2(array, _symbolsInSections, sizeof(uint32_t), &extra, &symbolIndexSorter);
